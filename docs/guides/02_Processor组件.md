...
| `processor-aggregate-group` | processor/aggregate | 分组聚合 | ❌ | ✅ |
...
## processor-aggregate-group

### 作用

对数据进行分组聚合，支持计数、求和等聚合操作。
...
聚合后的 `map[string]interface{}` 对象，包含以下字段：

| 字段 | 类型 | 说明 |
|------|------|------|
| _key | string | 分组键（由分组字段值拼接而成） |
| _count | int | 组内记录数 |
| {field}_sum | float64 | 字段的求和结果（如果配置了 sum 聚合） |
...
| aggregates | map | 否 | 聚合配置，字段名 -> 聚合操作 |
...
```yaml
processors:
  - name: processor-aggregate-group
    config:
      group_by:
        - department
      aggregates:
...
输入数据：
```json
{"department": "sales", "name": "Alice", "sales": 1000}
{"department": "sales", "name": "Bob", "sales": 1500}
{"department": "engineering", "name": "Charlie", "sales": 800}
```
...
输出结果：
```json
{"_key": "sales|", "_count": 2, "sales_sum": 2500}
{"_key": "engineering|", "_count": 1, "sales_sum": 800}
```

### 注意事项

- 聚合操作会在所有输入数据处理完成后才输出结果
- 分组键由各分组字段的值用 `|` 连接而成
- 求和操作仅支持数值类型字段
